...
drive="C:"
path.upto <- paste("Users","Nolan","Documents","School","STAT4601", sep="/" )
...
##
# plot4x4 <- function(km, d)
# Clustering/VQ/Image Compression 
# 250 © Mills2018
# Data Mining 2018
##
plot4x4km <- function(d, s) {
  set.seed(s)
  ...
  for (i in 2:14) {
    KM <- kmeans(d, i, 15)
    plot(KM$cluster, col=colors[KM$cluster], pch=KM$cluster, main=paste(i," clusters"))
    errs[i-1] <- sum(KM$withinss)
    DBI[i-1] <- Davies.Bouldin(KM$centers, KM$withinss, KM$size)
  }
  plot(2:14, errs, main=paste("SS", " With Seed:", s))
  lines(2:14, errs)
...
# Test Set
for (i in (1:10)) {
  plot4x4km(d.wine.testset, i)
}
# Training Set
for (i in (1:10)) {
  plot4x4km(d.wine.trainset, i)
}
##########################################################################
# - Question 3 -
